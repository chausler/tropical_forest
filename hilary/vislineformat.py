...
if __name__ == '__main__':
    tc = TopicChain('topic_keys.json', threshold=0.4, max_incoming=2, max_outgoing=1)
    topic_freqs = json.load(open('topic_freqs.json'))

    trees = extract(tc, 10)
    lst = []
    for (i, t) in enumerate(trees):
        vis = VisLineFormat(t, list(range(12)), topic_freqs)
        json_lst = vis.to_json_lst()
        lst.extend(json_lst)

    with open('/Users/ranxiao/Desktop/tropical_forest/visualization/lda_vis1.json', 'w') as outfile:
